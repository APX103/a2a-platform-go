...
| Failed proxy calls leave coherent task/trace state | architecture "task/message/trace audit chain" | Connection refusal, timeout, upstream 5xx, malformed SSE, and empty response each produce a task terminal state plus a response/error trace. | P0 | `internal/handler/handler_test.go::TestAgentProxyConnectionFailureRecordsErrorTrace`; `internal/handler/handler_test.go::TestAgentProxyConnectionFailureSanitizesTraceURL` | verified | `internal/handler` |
